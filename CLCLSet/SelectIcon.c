...
/* function prototype normally in CLCLSet.h, must not be included here 
 * because this would yield redefiniion of WM_LV_EVENT
 */
...
static BOOL CALLBACK select_icon_proc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_HELP:
		if (show_help(IDD_DIALOG_SELECT_ICON, (LPHELPINFO)lParam))
			return TRUE;
		break;
